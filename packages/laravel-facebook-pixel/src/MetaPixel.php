...
class MetaPixel
{
    ...
    public function setTestEventCode(string $testEventCode): void
    {
     ...
     */
    public function send(string $eventName, string $eventID, CustomData $customData, ?UserData $userData = null, ?string $eventSourceUrl = null): ?EventResponse
    {
        if (! $this->isEnabled()) {
            return null;
        }
        ...
        $event = (new Event)
            ...
            ->setEventTime(time())
            ->setEventId($eventID)
            ->setEventSourceUrl($eventSourceUrl ?? URL::current())
